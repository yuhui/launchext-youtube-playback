...
describe('videoDuration data element delegate', function() {
  var dataElementDelegate = require('../../../src/lib/dataElements/videoDuration');
  ...
  describe('with invalid "event" argument', function() {
    ...
    it(
      'should be undefined when "duration" property is missing',
      function() {
        delete this.event.youtube.duration;
        var result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
