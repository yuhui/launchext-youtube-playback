...
describe('videoMuted data element delegate', function() {
  var dataElementDelegate = require('../../../src/lib/dataElements/videoMuted');
  ...
  describe('with invalid "event" argument', function() {
    ...
    it(
      'should be undefined when "muted" property is missing',
      function() {
        delete this.event.youtube.muted;
        var result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
