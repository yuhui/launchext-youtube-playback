...
describe('moduleNames data element delegate', function() {
  var dataElementDelegate = require('../../../src/lib/dataElements/moduleNames');
  var getBaseEvent = require('../../specHelpers/getBaseEvent');

  beforeEach(function() {
    this.event = getBaseEvent(['moduleNames']);
  ...
  describe('with invalid "event" argument', function() {
    ...
    it(
      'should be undefined when "moduleNames" property is missing',
      function() {
        delete this.event.youtube.moduleNames;
        var result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
